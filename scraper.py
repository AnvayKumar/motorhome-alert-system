...
# These RSS endpoints bypass the Cloudflare browser verification walls completely
TRADEME_RSS = "https://www.trademe.co.nz/Browse/SearchResults.aspx?nav_perpage=50&cid=2983&user_type=dealer&price_max=100000&attribute_70_72=71&format=rss"
AUTOTRADER_RSS = "https://www.autotrader.co.nz/used-cars-for-sale/motorhomes/rss?priceTo=100000"
...
def check_marketplaces():
    ...
    try:
        ...
        print(f"TradeMe RSS Read Exception: {e}")

    # ---- 2. AUTOTRADER RSS ENGINE ----
    try:
        response = requests.get(AUTOTRADER_RSS, headers=headers, timeout=15)
        if response.status_code == 200:
            root = ET.fromstring(response.content)
            for item in root.findall(".//item"):
                title = item.find("title").text
                ...
                # Clean filter for 4-berth keywords in the title text
                if not any(x in title.lower() for x in ["2-berth", "2 berth", "3 berth", "2berth"]):
                    current_runs.append({"title": title, "link": link, "price": price, "source": "AutoTrader"})
    except Exception as e:
        print(f"AutoTrader RSS Read Exception: {e}")

    # Chronological sort and update
    new_discoveries = []
...
def generate_html(listings):
    ...
    if not listings:
        cards_html = "<div class='no-listings'>No active listings found right now. Checking again in 15 mins...</div>"
    else:
...
<head>
    ...
    <title>4-Berth Motorhomes Dashboard</title>
    <style>
